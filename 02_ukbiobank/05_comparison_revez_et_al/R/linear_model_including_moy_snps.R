...
# want to model jointly the snps:
# rs7041
# rs842998
# rs842873
# rs11731496
...
joint_model %>%
  qs::qsave(
    here::here("results", "2022_11_11_paper_review", "qs",
      "sccs_vdbp_joint_model_with_moy.qs"))

joint_model %>%
  dplyr::mutate(
    rsids = purrr::map_chr(rsids,
      ~ ifelse(is.null(.), "", str_c(., collapse = ",")))) %>%
  as.data.frame() %>%
  readr::write_tsv(
    here::here("results", "2022_11_11_paper_review", "tsv",
      "sccs_vdbp_joint_model_with_moy.tsv"))
