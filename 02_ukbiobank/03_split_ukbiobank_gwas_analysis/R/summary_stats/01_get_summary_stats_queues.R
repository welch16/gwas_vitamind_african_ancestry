...
out_dir <- here::here("results", folder, "plink2", "qc")
...
queue %<>%
  crossing(samples) %>%
  dplyr::mutate(
    outfile = glue::glue("{outdir}/ukbiobank_{pop}_gwas_{chr}",
      outdir = out_dir),
...
queue %>%
  ...
  readr::write_csv(
    here::here("results", folder, "condor", "summary_stats",
      "ukb_gwas_queue.csv"), col_names = FALSE)
