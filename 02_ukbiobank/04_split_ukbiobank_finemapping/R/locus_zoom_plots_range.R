...
locus_zoom_plot <- function(locus, index, db, stats, vcf_files, mart) {
  ...
  geno_ids <- colnames(geno_mat)

  stats %<>%
    dplyr::mutate(
      ld_ids = purrr::map(id, ~ str_subset(geno_ids, .)))

  ld_mat <- cor(geno_mat)

  stats %<>%
    dplyr::mutate(index = p == min(p))
  ...
  stats %<>%
    dplyr::filter(purrr::map_dbl(ld_ids, length) > 0) %>%
    dplyr::mutate(ld_ids = purrr::map_chr(ld_ids, 1))
  
  ld_vec <- ld_mat[stats$ld_ids, index_snp[[1]] ]

  stats %<>%
    dplyr::mutate(ld = ld_vec) %>%
    dplyr::mutate(ld = ifelse(is.na(ld), 0, ld))
...
top_snp %<>%
  dplyr::mutate(
    filename = glue::glue("{dr}/ukb_afroc_25hvd_{id2}_locuszoom_range.pdf",
      dr = pngdir),
